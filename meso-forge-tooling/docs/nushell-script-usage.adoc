...
|`RATTLER_AUTH_FILE`
|Authentication for rattler-build (not recommended, use keyring instead)
...
. *Authentication Issues*
   * Recommended: Use `pixi auth login` for keyring storage
   * Alternative: Ensure `RATTLER_AUTH_FILE` is set (not recommended)
